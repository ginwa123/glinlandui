...
// for the same reason `linux/shim.h` is: Zig's @cImport only ever sees this
// file, so the ObjC headers (and their macros) never have to survive
// translation. The implementation lives in `mac/shim.m`.
//
...
/// (see mac/present.zig). Safe to call with a NULL buffer or a zero
